...
Переходы срабатывают только в том случае, если DOM-операция была инициирована через Vue, либо посредством директивы (например `v-if`), либо посредством вызова одного из методов экземпляра (например `vm.$appendTo()`. {.tip}
